...
- [~] 2단계-b: 실데이터 연결 **16/17**
  ...
  - 남음: **매물량 1종** — 공개 데이터가 존재하지 않음(신고의무 없는 지표). 민간(아실·네이버)은
    `robots.txt`가 봇 접근을 명시 거부 → 자동 수집 안 함. `manual/flow-listing.csv`로만 채울 수 있고,
    비어 있으면 목업이 아니라 **'데이터 없음'**으로 표시된다. 상세: [manual/README](scripts/etl/manual/README.md)
